...
class Lane:
    ...
    Default constructor
        ...
        self.roi_points = np.float32([(180,200),(0,300),(width,308),(500,200)])
        #self.roi_points = np.float32([(50,50),(0,150),(width,160),(600,50)])
    ...
    def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
        ...
        try:
            ...
            left_fit=np.array([-8.60163623e-05 ,-100.72739030e-02  ,2.31176312e+02])
        ...
        try:
            ...
            right_fit=np.array([ 5.36325687e-07, -2.47381205e-01 , 10.94415614e+02])
    ...
    def get_lane_line_indices_sliding_windows(self, plot=False):
        ...
        try:
            ...
            right_fit=np.array([ 5.36325687e-07, -2.47381205e-01 , 5.94415614e+02])
    ...
    :param frame: The camera frame that contains the lanes we want to detect
        ...
        # Perform Sobel edge detection on the S channel of 
        ...
        _, sxbinary = edge.threshold(hsv[:, :, 1], thresh=(120, 255))
        sxbinary = edge.blur_gaussian(sxbinary, ksize=5) # Reduce noise
        ...
        # We expect lane lines to be nice, pure colors solid yellow
        # and have high saturation channel values.
        # s_binary is matrix full of 0s (black) and 255 (white) intensity values
        #value is 
        s_channel = hsv[:, :, 1] # use only the saturation channel data
        v_channel = hsv[:, :, 2] # use only the value channel data

        _, s_binary = edge.threshold(s_channel, (100, 255))
        _, v_binary = edge.threshold(v_channel, thresh=(60, 255))

        # Perform binary thresholding on the R (red) channel of the 
        # original BGR video frame. 
        # r_thresh is a matrix full of 0s (black) and 255 (white) intensity values
        # yellow in the regions with the richest red channel values (e.g. >80).
        # Pure yellow is bgr(0, 255, 255). 

        _, r_binary = edge.threshold(frame[:, :, 2], thresh=(80, 255))

        # Lane lines should be pure in color and have high red channel values 
        # Bitwise AND operation to reduce noise and black-out any pixels that
        # don't appear to be nice, pure, solid colors 
        sv_binary = cv2.bitwise_and(s_binary, v_binary)
        svr_binary = cv2.bitwise_and(sv_binary, r_binary)

        ### Combine the possible lane lines with the possible lane line edges ##### 
        # If you show rs_binary visually, you'll see that it is not that different 
        # from this return value. The edges of lane lines are thin lines of pixels.
        self.lane_line_markings = cv2.bitwise_or(svr_binary, sxbinary.astype(
                                                            np.uint8))   
...
def main():
    ...
    s.bind(('192.168.1.4',9999))   # This method will bind the sockets with server and port no
    ...
    vid = cv2.VideoCapture('http://admin:password@192.168.1.2:8000/stream.mjpg')
    ...
    while(True):
        ...
        #time.sleep(0.03)
        offset=lane_obj.calculate_car_position()

        offset=np.arctan(offset/180)*180/np.pi
        #results = model(frame)
        object0='NAN'

        #if (len(results.pandas().xyxy[0]['name'])>0 ):
        #    object0=results.pandas().xyxy[0]['name'][0]
        stroffset=str(round(offset))
        CMD=f"{stroffset} {object0}"
        c.send(CMD.encode('UTF-8'))  #Will send message 
        out.write(frame)
